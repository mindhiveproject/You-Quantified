 ...
 */
function DisplayVisResult({ visInfo, addReference, setShowModal, hasBeenAdded }) {
  function handleClick() {
    addReference({ name: visInfo?.title, id: visInfo?.id, type: "visual", codeURL: visInfo?.code?.url });
    setShowModal(false);
  }

  return (
    <button
      className="btn btn-outline-light btn-hover-primary w-100 text-white p-2 mb-1"
      onClick={handleClick}
    ...
    >
      <div className="d-flex p-0 pb-1 justify-content-between align-items-center">
        ...
        {hasBeenAdded && <span className="material-symbols-outlined">check_small</span>}
      </div>
